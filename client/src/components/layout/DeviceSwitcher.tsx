...
export default function DeviceSwitcher({ collapsed }: { collapsed: boolean }) {
  ...
  return (
    <div className="mt-4 pt-4 border-t border-zinc-800/60 space-y-1">
      {!collapsed && (
        <div className="flex items-center gap-1 text-[0.8125rem] text-zinc-500 mb-1 px-3">
...
function DeviceLink({ device, num, collapsed }: { device: Device; num: number; collapsed: boolean }) {
    ...
    <>
      ...
      {!collapsed && (
        <>
          ...
          {num <= 9 && (
            <span className="text-xs font-mono text-zinc-600 shrink-0">{num}</span>
          )}
        </>
      )}
    </>
  ...
  const cls = `relative flex items-center gap-3 ${collapsed ? 'justify-center px-0' : 'px-3'} h-10 rounded-md text-base transition-colors`;
